...
`quitte` package as a data object in the `quitte` package in case you just want
to try something out or give an example for something.  So we load it like this:
...
In this case, values below 0 or might be meaningless. So use spline extensions
with care.
...
> (`::`) operator.  This is only needed in this vignette, since `quitte` can't
> import the `mip` package.)
